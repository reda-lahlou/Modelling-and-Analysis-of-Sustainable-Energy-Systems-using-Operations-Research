...
e(i)             emission cost
p_var_ini(i,t)   production of variable techs in a year (before investment)
p_tot(t)         total production from power plants
...
$onEcho > howToRead_extension_model_sensitivity_analysis_to_Excel_test.txt
var = C.l rng=With_EV!B2
squeeze=n var = new_cap.l rng=With_EV!B5
EpsOut=0 var = p.l cdim=0 rng=With_EV!E1
$offEcho

execute_unload "extension_model_sensitivity_analysis.gdx" C.l p.l new_cap.l
execute 'gdxxrw.exe extension_model_sensitivity_analysis.gdx o=extension_model_sensitivity_analysis_test.xls @howToRead_extension_model_sensitivity_analysis_to_Excel.txt'

*INGRID ADD
PARAMETERS
p_dep_eff(i,l,t)         Production depending on efficiency
C_dep_eff(l)             Cost depending on efficiency
new_cap_dep_eff(i,l)     New capacity dependent on efficiency
rep_dep_eff(i,l)         Repartition  dependent on efficiency
storageunload_dep_eff(t,l,k)
storageload_dep_eff(t,l,k)


loop(l,car_unload_cap=(ord(l))/100;
car_load_cap=(ord(l))/100;
SOLVE ED using LP minimizing C;
p_dep_eff(i,l,t)=p.l(i,t);
C_dep_eff(l)=C.l;
new_cap_dep_eff(i,l)=new_cap.l(i);
storageunload_dep_eff(t,l,k)=storageunload.l(t,k);
storageload_dep_eff(t,l,k)=storageload.l(t,k);
Display car_load_cap
);

new_cap_dep_eff(i,l)$(not new_cap_dep_eff(i,l)) = eps;

loop((i,l),rep_dep_eff(i,l)=Sum(t,p_dep_eff(i,l,t))/(Sum(t,d_down(t))+ Sum((t,k),dcars(t,k)*n_cars(k)-(storageunload_dep_eff(t,l,k)-storageload_dep_eff(t,l,k))*n_cars(k))));

Display new_cap_dep_eff

$onEcho > howToRead_extension_model_to_Excel_efficiency.txt
par = rep_dep_eff cdim=1 rdim=1 rng=Prod_dep_efficiency_both!A1
par = C_dep_eff rng=Cost_dep_efficiency_both!A1
EpsOut=0 par = new_cap_dep_eff cdim=1 rdim=1 rng=New_cap_dep_efficiency_both!A1
$offEcho

execute_unload "extension_model.gdx" C_dep_eff new_cap_dep_eff rep_dep_eff
execute 'gdxxrw.exe extension_model.gdx o=extension_model.xls @howToRead_extension_model_to_Excel_efficiency.txt'
